...
KNLMeansClass::KNLMeansClass(PClip _child, const int _D, const int _A, const int _S, const int _wmode, const double _h,
	const char* _ocl_device, const bool _lsb, const bool _info, IScriptEnvironment* env) : GenericVideoFilter(_child),
	D(_D), A(_A), S(_S), wmode(_wmode), h(_h), ocl_device(_ocl_device), lsb(_lsb), info(_info) {
	...
	// Checks user value.
	if ((!vi.IsPlanar() || !vi.IsYUV()) && !vi.IsRGB32()) env->ThrowError("KNLMeansCL: planar YUV data or RGB32!");
	if (vi.IsRGB32() && lsb) env->ThrowError("KNLMeansCL: RGB64 is not supported!");
	if (D < 0) env->ThrowError("KNLMeansCL: D must be greater than or equal to 0!");
	if (A < 0) env->ThrowError("KNLMeansCL: A must be greater than or equal to 0!");
	...
	const cl_image_format image_format = { vi.IsRGB32() ? CL_BGRA : CL_LUMINANCE, lsb ? CL_UNORM_INT16 : CL_UNORM_INT8 };
	...
	mem_in[0] = clCreateImage2D(context, CL_MEM_READ_ONLY, &image_format, image_dimensions[0], image_dimensions[1], 0, NULL, NULL);
	if (D) mem_in[1] = clCreateImage2D(context, CL_MEM_READ_ONLY, &image_format, image_dimensions[0], image_dimensions[1], 0, NULL, NULL);
	mem_out = clCreateImage2D(context, CL_MEM_WRITE_ONLY, &image_format, image_dimensions[0], image_dimensions[1], 0, NULL, NULL);
	...
	snprintf(options, 2048, "-cl-denorms-are-zero -cl-fast-relaxed-math -Werror \
        ...
        H_BLOCK_X, H_BLOCK_Y, V_BLOCK_X, V_BLOCK_Y, vi.IsRGB32(), S, wmode, D, 65025.0 / (h*h*(2 * S + 1) * (2 * S + 1)));
	...
	ret |= clSetKernelArg(kernel[1], 0, sizeof(cl_mem), &mem_in[0]);
	ret |= clSetKernelArg(kernel[1], 1, sizeof(cl_mem), &mem_in[D ? 1 : 0]);
...
// VapourSynthInit
#ifdef VAPOURSYNTH_H
static void VS_CC knlmeansInit(VSMap *in, VSMap *out, void **instanceData, VSNode *node, VSCore *core, const VSAPI *vsapi) {
...
PVideoFrame __stdcall KNLMeansClass::GetFrame(int n, IScriptEnvironment* env) {
	...
	if (lsb) {
		...
		const uint8_t *msbp = src->GetReadPtr(PLANAR_Y);
		const uint8_t *lsbp = msbp + image_dimensions[1] * src->GetPitch(PLANAR_Y);
		...
		ret |= clEnqueueWriteImage(command_queue, mem_in[0], CL_TRUE, origin, region,
			image_dimensions[0] * sizeof(uint16_t), 0, buffer, NULL, NULL, NULL);
	} else {
		// 8-bits.
		ret |= clEnqueueWriteImage(command_queue, mem_in[0], CL_TRUE, origin, region,
			src->GetPitch(PLANAR_Y), 0, src->GetReadPtr(PLANAR_Y), NULL, NULL, NULL);
	}
	...
	if (lsb) {
		...
		// 8-bits.
		ret |= clEnqueueReadImage(command_queue, mem_out, CL_TRUE, origin, region,
			dst->GetPitch(PLANAR_Y), 0, dst->GetWritePtr(PLANAR_Y), NULL, NULL, NULL);
	}
	...
	if (info) {
		...
		snprintf(buffer, 2048, " Iterations: %i", ((2 * D + 1)*(2 * A + 1)*(2 * A + 1) - 1) / (D ? 1 : 2));
		DrawString(frm, pitch, 0, y++, buffer);
...
static const VSFrameRef *VS_CC VapourSynthPluginGetFrame(int n, int activationReason, void **instanceData,
	...
	if (activationReason == arInitial) {
		...
		if (d->vi->format->colorFamily == cmYUV || d->vi->format->colorFamily == cmYCoCg) {
			vs_bitblt(vsapi->getWritePtr(dst, 1), vsapi->getStride(dst, 1), vsapi->getReadPtr(src, 1), vsapi->getStride(src, 1),
				vsapi->getFrameWidth(src, 1) * d->vi->format->bytesPerSample, vsapi->getFrameHeight(src, 1));
			vs_bitblt(vsapi->getWritePtr(dst, 2), vsapi->getStride(dst, 2), vsapi->getReadPtr(src, 2), vsapi->getStride(src, 2),
				vsapi->getFrameWidth(src, 2) * d->vi->format->bytesPerSample, vsapi->getFrameHeight(src, 2));
		}
		...
		if (d->d) {
			...
			for (int k = int64ToIntS(-d->d); k <= d->d; k++) {
				...
				ret |= clEnqueueWriteImage(command_queue, d->mem_in[1], CL_TRUE, origin, region, vsapi->getStride(src, 0),
...
KNLMeansClass::~KNLMeansClass() {
	...
	if (D) clReleaseMemObject(mem_in[1]);
...
static void VS_CC VapourSynthPluginFree(void *instanceData, VSCore *core, const VSAPI *vsapi) {
	...
	if (d->d) clReleaseMemObject(d->mem_in[1]);
...
AVSValue __cdecl AviSynthPluginCreate(AVSValue args, void* user_data, IScriptEnvironment* env) {
	return new KNLMeansClass(args[0].AsClip(), args[1].AsInt(DFT_D), args[2].AsInt(DFT_A),
		args[3].AsInt(DFT_S), args[4].AsInt(DFT_wmode), args[5].AsFloat(DFT_h),
		args[6].AsString(DFT_ocl_device), args[7].AsBool(DFT_lsb), args[8].AsBool(DFT_info), env);
}
#endif //__AVISYNTH_6_H__
...
static void VS_CC VapourSynthPluginCreate(const VSMap *in, VSMap *out, void *userData, VSCore *core, const VSAPI *vsapi) {
	...
	ret |= clSetKernelArg(d.kernel[1], 0, sizeof(cl_mem), &d.mem_in[0]);
	ret |= clSetKernelArg(d.kernel[1], 1, sizeof(cl_mem), &d.mem_in[d.d ? 1 : 0]);
	...
	*data = d;
	vsapi->createFilter(in, out, "KNLMeansCL", knlmeansInit, VapourSynthPluginGetFrame, VapourSynthPluginFree, fmParallelRequests, 0, data, core);
}
#endif //__VAPOURSYNTH_H__
...
extern "C" __declspec(dllexport) const char* __stdcall AvisynthPluginInit3(IScriptEnvironment* env, const AVS_Linkage* const vectors) {
	AVS_linkage = vectors;
	env->AddFunction("KNLMeansCL", "c[D]i[A]i[S]i[wmode]i[h]f[device_type]s[lsb_inout]b[info]b", AviSynthPluginCreate, 0);
...
VS_EXTERNAL_API(void) VapourSynthPluginInit(VSConfigPlugin configFunc, VSRegisterFunction registerFunc, VSPlugin *plugin) {
	...
	registerFunc("KNLMeansCL", "clip:clip;d:int:opt;a:int:opt;s:int:opt;wmode:int:opt;h:float:opt;device_type:data:opt;info:int:opt",
